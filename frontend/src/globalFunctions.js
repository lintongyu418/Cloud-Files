...
const globalFunction = {
  ...
  getImgMinPath: function (row) {
    ...
    if (row.fileUrl) {
      if (row.isOSS === 1) {
        // 阿里云OSS对象存储
        fileUrl =
          `https://${Cookies.get('viewDomain')}${row.fileUrl}?x-oss-process=image/resize,m_fill,h_150,w_150/rotate,0`
      } else {
        // 本地磁盘存储
        const index = row.fileUrl.lastIndexOf('.')
  ...
  getViewFilePath: function (row) {
    return `/api/file/preview?userFileId=${row.userFileId}`
  },
  /**
   * 获取文件下载路径
   * @param {object} row 文件信息
  ...
  getDownloadFilePath: function (row) {
    return `/api/file/download?userFileId=${row.userFileId}`
  },
  /**
   * 获取 office 文件在线预览路径
   * @param {object} row
