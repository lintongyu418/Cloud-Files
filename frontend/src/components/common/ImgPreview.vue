<template>
  <transition name="el-fade-in-linear el-fade-in">
    ...
    >
      <!-- 顶部信息栏 & 工具栏 -->
      <div v-if="imgPreviewVisible" class="tip-wrapper">
        <div :title="activeFileName + activeExtendName" class="name">{{ activeFileName }}.{{ activeExtendName }}</div>
        <div class="opera-btn-group">
          <el-input-number v-model="inputActiveIndex" :min="1" :max="imgPreviewList.length" size="mini"/>
          <span class="split-line">/</span>{{ imgPreviewList.length }}
        </div>
        <div class="tool-wrapper">
          <i class="item el-icon-refresh-right" title="向右旋转" @click="rotateImg"/>
          <a
          ...
          <el-tooltip effect="dark" placement="bottom">
            ...
            <div class="item text-wrapper" style="margin-right: 20px">
      ...
      >
      <!-- 左右切换图标 -->
      <i v-show="activeIndex > 0" class="pre-icon el-icon-arrow-left" title="上一张" @click.stop="activeIndex--"/>
      <i
